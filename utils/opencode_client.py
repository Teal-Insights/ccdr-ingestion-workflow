...
This module provides a reusable client for interacting with the OpenCode API service,
...
class OpenCodeClient:
    """
    Client for interacting with the OpenCode API service.
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, cleanup_on_exit: bool = False):
        ...
        self.base_url = (base_url or os.getenv("OPENCODE_BASE_URL", "")).rstrip("/")
        
        if not self.api_key or not self.base_url:
            raise ValueError("OPENCODE_API_KEY and OPENCODE_BASE_URL must be set")
    ...
    def create_session(self, files: List[FileInput], ttl_seconds: int = 3600) -> str:
        ...
        """
        logger.info("Creating session on OpenCode API service...")
    ...
    def run_job(self, prompt: str, model: str = "openai/gpt-5", timeout_s: int = 600) -> str:
        """
    ...
    def wait_for_job_completion(self, job_id: str, timeout_s: int = 600, poll_interval: int = 5) -> JobStatusResponse:
        ...
        while True:
            ...
            if job_status.stdout:
                logger.info(f"Job stdout: {job_status.stdout}")
    ...
    def get_session_results(self) -> SessionResultsResponse:
        ...
        if session_results.result:
            logger.info(f"Response from OpenCode API service: {session_results.result}")
    ...
    def execute_restructuring_job(
        ...
        Args:
            ...
            model: AI model to use for the job
    ...
    def execute_fixup_job(
        ...
        Args:
            ...
            output_file: Name of the expected output file
            config_files: Additional configuration files to include
            model: AI model to use for the fixup

...
def validate_block_coverage(
    header_blocks: List[int],
    main_blocks: List[int],
    footer_blocks: List[int],
    total_blocks: int,
) -> None:
    """
...
async def detect_structure_with_llm(
    html_content: str, api_key: str
) -> DocumentStructure:
    """
...
Respond with a JSON object containing the three sections."""
    ...
    response = await loop.run_in_executor(
        ...
        partial(
            ...
            response_format={
                "type": "json_object",
                "response_schema": DocumentStructure.model_json_schema(),
            },
        ),
    )
...
def filter_blocks_by_numbers(
    blocks: List[Block], block_numbers: List[int]
) -> List[Block]:
    """
...
async def detect_structure(
    ...
    if len(blocks_doc.blocks) != total_blocks:
        raise ValueError(
            f"Block count mismatch: HTML has {total_blocks} blocks, JSON has {len(blocks_doc.blocks)} blocks"
        )
    ...
    if api_key:
        ...
        structure = DocumentStructure(header="", main=f"1-{total_blocks}", footer="")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: uv run detect_structure.py <html_file> <blocks_json> <output_dir>"
        )

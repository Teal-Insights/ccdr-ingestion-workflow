...
BASE_URL = os.getenv("OPENCODE_BASE_URL", "").rstrip("/")

if not API_KEY or not BASE_URL:
    raise ValueError("OPENCODE_API_KEY and OPENCODE_BASE_URL must be set in .env file")
...
def restructure_with_opencode(
    ...
    """
    Restructure HTML using OpenCode API service.
    ...
    Args:
        ...
        timeout_seconds: Timeout for the OpenCode job (default 30 minutes for complex documents)
...
{HTML_PROMPT}"""
    ...
    # Load the validate_html.ts file
...
#            FileInput(path=".opencode/plugin/validate_html.ts", content=validate_html_content)
    ...
    try:
        ...
        while True:
            ...
            stdout = job_status.stdout
            if stdout:
                logger.info(f"Job stdout: {stdout}")
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    try:
        structured_nodes = restructure_with_opencode(
            content_blocks,

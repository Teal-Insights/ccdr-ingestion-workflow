...
class BaseBlock(BaseModel, ABC):
    ...
    id: Optional[int] = Field(
        default=None, description="Optional unique identifier for the block"
    )
    block_type: str = Field(description="Type of the block (text, image, svg, etc.)")
...
class TextBlock(BaseBlock):
    ...
    block_type: Literal["text"] = Field(
        default="text", description="Type of the block, always 'text' for text blocks"
    )
    text: str = Field(
...
class BlocksDocument(BaseModel):
    ...
    total_pages: int = Field(
        ge=1, description="Total number of pages in the PDF document"
    )
    total_blocks: int = Field(
        ge=0, description="Total number of blocks extracted from all pages"
    )
